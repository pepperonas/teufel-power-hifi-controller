...
// LED-Matrix-Modus (Aus/dB/BPM) + Iris-Overlay auf dem Arduino R4
app.get("/api/matrix", async (req, res) => {
    try {
        const r = await bridgeRequest("MATRIX?");          // "OK <mode> iris=0|1"
        const rest = r.replace(/^OK\s*/, "").trim();
        const parts = rest.split(/\s+/);
        const mode = parts[0] || "off";
        let iris = false;
        for (const p of parts.slice(1)) {
            const m = /^iris=(\d+)$/i.exec(p);
            if (m) iris = m[1] === "1";
        }
        res.json({ mode, iris });
...
app.get("/api/matrix/iris", async (req, res) => {
    try {
        const r = await bridgeRequest("IRIS?");
        const on = /OK\s*1/.test(r);
        res.json({ iris: on });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post("/api/matrix/iris", async (req, res) => {
    const body = req.body || {};
    const enabled = body.iris === true || body.iris === 1 || body.iris === "1"
        || body.enabled === true || body.enabled === 1 || body.enabled === "on";
    const explicitOff = body.iris === false || body.iris === 0 || body.iris === "0"
        || body.enabled === false || body.enabled === 0 || body.enabled === "off";
    const on = explicitOff ? false : enabled;
    try {
        const r = await bridgeRequest("IRIS " + (on ? "on" : "off"));
        if (r.startsWith("OK")) res.json({ success: true, iris: on });
...
app.post("/api/matrix", async (req, res) => {
    ...
    if (!["off","db","pegel","bpm","smiley","vu","heart","spektrum","welle","temp","humidity","clock","analog"].includes(mode)) return res.status(400).json({ error: "Invalid mode" });
    try {
